...
class Epoch:
    ...
    def _to_device(self):
        ...
        # Metrics (ConfusionMatrixMetric) don't need .to() as they use numpy/cpu tensors
    ...
    def run(self, dataloader):
        ...
        # Reset confusion matrix metrics at start of epoch
        ...
        with tqdm(dataloader, desc=self.stage_name, file=sys.stdout, disable=not (self.verbose)) as iterator:
            for (x1, x2, y, filename) in iterator:
                ...
                loss_logs = {'cross_entropy_loss': loss_meter.mean}
                logs.update(loss_logs)

                # update metrics logs (for tqdm progress display, use current computed value)
                for metric_fn in self.metrics:
                    metric_value = metric_fn(y_pred, y).detach().cpu().numpy()
                    metrics_meters[metric_fn.__name__].add(metric_value)
                metrics_logs = {k: v.mean for k, v in metrics_meters.items()}
        ...
        # For confusion matrix metrics, the final value is the last computed score (after all batches)
        # Since forward() accumulates into confusion_matrix and returns running score,
        # the final call returns the score based on all epoch data
        for k in metrics_meters:
            metrics_meters[k].avg = metrics_meters[k].val  # Use final value, not average
        metrics_logs = {k: v.avg for k, v in metrics_meters.items()}
        logs.update(metrics_logs)
